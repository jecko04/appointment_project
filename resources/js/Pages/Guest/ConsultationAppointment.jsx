  ...
  const submit = async (e) => {
    e.preventDefault();
      ...
      try
      {
        ...
        const response = await fetch(route('guest.consultation.store'), {
          method: 'POST',
          headers: {
...
const handleQRCode = (value) => {
  ...
  useEffect(() => {
    const firstService = categories.find(
      (category) =>
        category.Branch_ID === data.selectedBranch &&
        (category.Title === "Consultation" || category.Title === "consultation")
    );
    
    if (firstService) {
      handleServiceChange(firstService.Categories_ID);
    }
                  ...
                  <form onSubmit={submit} className="py-5 lg:py-10 flex flex-col items-start px-2 md:gap-2">
                    {currentStep === 0 && (
                      ...
                      <div className="flex flex-col md:flex md:flex-row p-3 lg:p-5 md:gap-10 lg:gap-20 2xl:gap-32 lg:shadow-md rounded-lg bg-white">
                        ...
                        <span className="text-[#2938DA]">Appoint</span>
                        <span className="text-[#2938DA]">Consultation</span>
                        <span className="text-[#ff4200]"> Services Here!</span>
                      </div>
                        ...
                        {data.selectedBranch && categories.filter(category => category.Branch_ID === data.selectedBranch && 
                          (category.Title === "Consultation" || category.Title === "consultation")
                        ).length === 0 && (
